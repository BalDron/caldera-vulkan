...
namespace veng {
    ...
    void MoveWindowToMonitor(gsl::not_null<GLFWwindow*> window, gsl::not_null<GLFWmonitor*> monitor) {
        ...
        const glm::ivec2 window_new_posision = GetMonitorPosition(monitor) + ( GetMonitorWorkareaSize(monitor) / 2) - (window_size / 2);
        glfwSetWindowPos(window, window_new_posision.x, window_new_posision.y);
    }
